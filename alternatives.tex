...
i.e., there is only a finite number of values $\theta_1, \ldots, \theta_m$ the parameter $\theta$ can assume,
...
\begin{align*}
\mathcal{M} &= \{ p(\vartheta) | \ul{p}_j \le p(\vartheta_j) \le \ol{p}_j\ \forall j=1,\ldots,m \}\,,
\end{align*}
where $\ul{p}_j$ and $\ol{p}_j$ are the lower and upper (indirectly) elicited bounds for $p(\vartheta_j)$, respectively.
As a summary measure for imprecision, Whitcomb defines $\Delta = \sum_{j=1}^m \ol{p}_j-\ul{p}_j$.

One example considers a reliability analysis problem
regarding the mean time to failure $\theta$ of a technical component \parencite[\S 4.1]{2005:whitcomb} %(\S 4.1),
and the influence of several hypothetical data sets on posterior imprecision is studied.
...
derived from the prior probability specifications.}
and in conflict with the elicited prior
are considered, with three sample sizes $n=2,6,10$ in each group.
...
or could be an artefact of the relatively low number of distinct parameter values $m$.

\textcite[\S 4]{2011:krautenbacher} compared the results from this example %\parencite{2005:whitcomb}
with a model of the framework from Section~\ref{sec:generalmodel} (model type \ref{enum:rectangular}).
To derive the prior parameter sets $\PZ$ from the expert assessments given by \textcite[Table~I]{2005:whitcomb} in terms of $\theta$,
...
His results are generally similar to those of \parencite[\S 4.1]{2005:whitcomb},
...
somewhat different from the imprecision $\Delta$ based on $\theta$.
...
with $[\Eu[\lambda]-\El[\lambda]$ decreasing according to sample size.
For the prior-data conflict situations, however, similarities vanish:
while $\Delta$ increased for $n=2$ and $n=6$, and dropped for $n=10$,
$[\Eu[\lambda]-\El[\lambda]$ is decreasing with $n$ just like in the prior-data agreement case,
...
Due to the low number of distinct parameter values in Whitcomb's model,
unions of highest density intervals for $\lambda$ can be determined only very coarsely.
%For these, the picture is more as expected:
Here, the picture is righted: starting from length $2.00$ of the prior HD interval,
HD intervals get shorter with growing sample size in both groups,
...
Expectation intervals for the conjugate-based model behave more as expected.
Krautenbacher derived the prior parameter set as $\PZ = [2.39, 2.85] \times [2.91, 4.08]$;
however, the set of priors $\MZ$ based on $\PZ$ fit the constraints posed by prior probability intervals
not very well \parencite[see][Abb.~20]{2011:krautenbacher}.
The conjugate Gamma distributions do not seem to capture well the prior information in this case.
Nevertheless, imprecision as measured by $[\Eu[\lambda]-\El[\lambda]$ decreases with sample size,
with slighly higher imprecision in the prior-data conflict case as expected.
...
we think that this strange result can be appropriated
to peculiarities of the Gamma distribution.%
\footnote{While for the prior variance holds $\V(\lambda) \in [0.028, 0.070]$,
the variance for the prior-data conflict posterior with $n=10$ ranges in $[0.120, 0.263]$,
such that the upper variance almost quadruples, leading to this overly wide posterior HD interval.}
...
with the discrete model showing some strange behaviour with respect to the expectation intervals,
and the conjugate model with respect to HD intervals.

***Basically similar results, no parametric assumption, but we don't really know what's going on!
(repair by complex pre-posterior analysis)
...
basic examples often do not use parametric distributions, but spaces
with only a few events.
with very small support.
Then, parametric models are unnecessary,
