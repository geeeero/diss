...
It has the convenient property of invariance under Bayesian updating,
...
for and sample size $n$ \textcite[\S 4.2.2]{2011:rinderknecht:diss}.
This is in strong contrast to the behaviour of the models discussed in Section~\ref{sec:generalmodel},
where $\MZ$ converges to a precise distribution for $n \to \inf$. 
The density ratio class has other advantageous properties,
...
(see the elicitation method mentioned in footnote~\ref{foot:rinderknecht}, page~\pageref{foot:rinderknecht}).
...
for probabilistic predictions are not exact, but provide a conservative approximation for
...
a model for prediction of a certain type of river biomass, depending on six model parameters.
...
such that computational burden is only marginally more extensive than in case of a precise prior.
...
\footnote{The marginal posterior lower bounding functions $l(\vartheta\mid\x)$ are indistinguishable
from zero if plotted in the same coordinate system as its respective the upper bounding functions $u(\vartheta\mid\x)$,
...
to model posterior imprecision as a function of sample size or \pdc.
%there seems to be no consistent mechanism by which posterior imprecision
The model is thus inable to model prior near-ignorance, and to reflect \pdc\ in posterior inferences.%
...
There are of course many other models that involving*** sets of priors,
especially in the robust Bayesian approach \parencite[see, e.g.,][]{1994:berger,2000:rios}.
There, typically so-called \emph{neighborhood models} are considered,
...
The idea in the robust Bayesian approach is to ensure robustness of the Bayesian analysis based on a single prior $\p_0$,
by checking that small deviations from $\p_0$ do not lead to large deviations in posterior inferences.
***also called systematic sensitivity analysis***

***from \textcite[\S 2.2]{itip-statinf}\\
Imprecise probabilities offer a formal, not casuistic framework for such \emph{Bayesian sensitivity analysis},
also containing the so-called \emph{robust Bayesian approach}.

As mentioned in Section~\ref{sec:bayesiansensitivity}, ***
***different interpretation, statinf par***
...
There are many different kinds of neighbourhood models
...
$100\cdot\varepsilon$\% of the data is distributed according to some distribution from a set $\mathcal{Q}$, %according to some other law,
...
As another example for a neighbourhood model, consider the \emph{odds-ratio model}.
...
some of which can be seen as special cases of the approach by \textcite{2011:rinderknecht:diss},
***and they make the distinction of near-noninformative versus informative (with \pdc\ sensitivity) priors.
