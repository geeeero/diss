...
\paragraph{Decomposition Including an Estimate of $\sigma^2$ Through the Null Model.}
...
\paragraph{Decomposition with Estimates of $\sigma^2$ Through Prior and Posterior Residuals.}
...
being the most important distribution for inference, is a multivariate $t$
...
\begin{align}
\V(\bbeta\mid \z)
                   &\stackrel{\phantom{(\ref{equ:cccp-sigma1})}}{=}
  \frac{\tbn}{\tan - 1} \tMn %
     = \E[\sigma^2\mid \z]\, \frac{n}{\nn}(\XtX)^{-1} \label{equ:cccp-varbeta} \\
\nonumber\\
% ------------- 4.2.1 -------------
                   &\stackrel{(\ref{equ:cccp-sigma1})}{=}
...
Just as in the SCP model, $\V(\beta\mid \z)$ can increase if the automatic abatement of the elements
