...
Now, we will present some of ideas for further developments within the framework
described in Section~\ref{sec:basicsetting}, i.e.,
involving sets of priors, defined using canonical conjugates,
...
Another important area for further development concerns the case of multidimensional $\yz$.
...
we will describe a novel idea for generating parameter prior sets $\PZ$
...
but it may nevertheless be quite difficult to elicit more general set \emph{shapes} and ascertain their consequences.
...
giving the (prior) expectation, or a prior guess, for the mean sample statistic.
%rays of constant expectation (i.e., $\approx \yn$)
Naturally, $\eta_1$ cannot have this property,
but in the transformed space for the Beta-Binomial model, the points on a ray emanating from the coordinate $(-2,0)$
...
a set shape in this parametrisation that allows for both \pdc\ sensitivity
...
and we are able to offer a solution that allows to remain within the generalised Bayesian framework,
using the Generalsied Bayes' Rule for updating.

Our preliminary studies show some very appealing results in case of the Beta-Binomial model,
for which the parametrisation is discussed in Section~\ref{sec:boatshape}.
