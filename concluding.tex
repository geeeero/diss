...
it is an approach going beyond the model framework of Section~\ref{sec:generalmodel},
...
the posterior based on $\mathcal{M}^{(n+m)}$ resonates this by increased imprecision,
...
In fact, such a parametrisation has been developed by Mi\c{k}elis Bickis (personal communication),
...
where $\eta_1$ essentially replaces the main prior parameter $\yz$,
while $\eta_0$ is just a shifted version of $\nz$.%
...
the representation of priors through this parametrisation allows to tailor shapes for desired inference behaviour much easier,
...
We are thus able to offer a solution ti this issue that allows to remain within the generalised Bayesian framework
by using the Generalised Bayes' Rule for updating.

Our preliminary studies show some very appealing results in case of the Beta-Binomial model,
...
and a joint publication of Mi\c{k}elis Bickis, Frank Coolen and the author of this thesis is planned
...
Regarding the criticisms discussed in Section~\ref{sec:updating},
...
but mirror this conflict by increased imprecision.
...
Another critique based on the presumed inflexibility of the Generalised Bayes' Rule
relates to posterior inferences being often `too imprecise'.
For the case of near-noninformative priors, this has been confronted
...
Although starting from a set of priors expressing near-ignorance.
...
may be able to fend off this criticism by
...
should remind us that using the Generalised Bayes' Rule
can lead to non optimal decision functions and thus, as one could argue,
also not to optimal inference procedures.%
...
that could lead to an interesting model combining sophisticated elicitation with reasonable handling of imprecision,
we think that a very attractive approach to inference where prior information can be included in the reasoning
is the model by \textcite{2008:cattaneo} in the variant that
allows to incorporate prior information by using a so-called prior likelihood.
Along the lines of \textcite{2011:4:isipta},
it would even be possible to apply the hierarchical level to models of the framework of Section~ref{generalmodel}.
An approach that we studied in more detail is weighting model from Section~\ref{sec:weightedinf}.
It was motivated by the dual role of $\nz$ in the framework from Section~\ref{generalmodel},
controlling both posterior precision and derivation from prior assignments
that can lead to unintuitive results in case of strong prior data agreement (see the discussion in Section~\ref{sec:ibbm-resume}).
...
To model this, one could consider allowing the prior strength parameter $\nz$
to take on negative values, opening up a rich field for research and discussions.


We hope that further studies like, e.g., those trying to refine the concept of coherence
towards allowing very substantial revisions in case of surprising data,%
...
we have to be able to model situations
