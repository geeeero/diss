...
As an example, if a subject assesses $\El$ such that $\El[X] = \El[Y] = \frac{1}{2}$,
and also that $\El[X+Y] = \frac{1}{2}$, $\El$ avoids sure loss, but is not coherent,
because one can imply from $\El[X] = \El[Y] = \frac{1}{2}$ that
$\El[X+Y]$ must be larger than $\frac{1}{2}$ \parencite[p.~67]{1991:walley}.
...
buying both of them at once makes the transaction less risky than buying the two separately,
