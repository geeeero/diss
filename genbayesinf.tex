...
briefly discussed below and in more detail in Sections~*** and ****.
...
A more detailed discussion on the issue of prior-data conflict will be given in Section~\ref{sec:gbicp-properties-criteria},
...
The approaches mentioned in (a) -- (d) are now discussed in detail,
...
\subsubsection{The IDM and other Prior Near-Ignorance Models}
...
\subsubsection{Substantial Prior Information and Sensitivity to Prior-Data Conflict}
...
Generalizing the IDM approach to conjugate priors for sample distributions of the canonical form \eqref{eq:expofam-sampledens},
...
should be based on parameter sets $\PZ = [\nzl, \nzu] \times \YZ$,
...
The model with $\PZ = \NZ \times \YZ$ is sensitive to prior-data conflict,
...
As mentioned in Section~\ref{sec:basicsetting}, item~\eqref{enum:rectangular}, page~\pageref{enum:rectangular},
the parameter set $\PN$ resulting from the update step \eqref{eq:canonicalupdate} is not a Cartesian product of $\YN$ and $\NN$,
...
If, e.g., $\nn\yn$ must be minimized to determine the posterior lower bound of a characteristic of interest,
...
In principle, $\PZ$ could have any form fitting the prior information at hand.
...
as it may heavily influence the posterior inferences (see Section~**** and \cite{Walter2011a}),
...
imprecise models allow for adequate modeling of partial information and prior-data conflict sensitivity,
