...
\[
\vec{X} = (X_1, \ldots, X_n)^T %\label{120722-1}
\]
...
\footnote{\cite{1987:gillies, 2000:gillies} argues that Bayes' Theorem was actually developed
in order to confront the problem of induction as posed by \cite{1739:hume}.}
...
Next, we will briefly describe a systematic of inference tasks.
...
(see, e.g., \cite[\S 5.1.5, p.~257]{2000:bernardosmith}, or \cite[\S 4.1.2, p.~166]{2007:robert}).
For a $\be(\nn\yn, \nn(1-\yn))$, the mode is
\begin{align*}
\mode p(\theta\mid\nn, \yn) &= \frac{\nn\yn - 1}{\nn - 2} = \frac{\nz\yz - 1 + s}{\nz - 2 + n}\,,
\end{align*}
...
whereas the \emph{error of the second kind}, denoted by $\beta$, of accepting $H_0$ although it is false, may be very large.}
...
to each of which a prior probability is assigned \cite[\S 5.2.4]{2007:robert}.
An overview on Bayesian testing, including a detailed comparison with classical testing procedures,
is given in \cite[\S 5.2--5.4]{2007:robert}.
...
\footnote{See, e.g., \cite[\S 5.2.2, Def.~5.2.5, p.~227]{2007:robert}, or \cite[p.~776]{1995:kass-raftery}.}
\begin{align*}
...
Improper priors are problematic in Bayesian testing \cite[\S 5.2.5]{2007:robert}
and should be avoided for parameters the test decides upon \cite[p.~782]{1995:kass-raftery}.
...
\end{align*}
with the the elements of the Jacobian matrix $\dfrac{\dd\bpsi}{\dd\btheta}$ being
\begin{align*}
...
and thus for the safety systems of nuclear power plants in general \cite[pp.~1, 4]{csni86-115},
...
%**EPRI NP-2433 ??***.
(see, e.g., \cite[pp.~121, 145]{csni86-115}).
...
ceased to function due to a tsunami wave flooding the rooms where they were installed \cite[p.~31]{2011:iaea::report}.
...
\begin{equation}
 ...
 \end{split}
\end{equation}
For our example, this means that $\nz=10$ \cite[p.~400, \S 3]{2011:kelly:atwood}.
...
\cite{1996:walley::idm} proposes as a so-called \emph{near-ignorance prior}%
