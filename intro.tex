...
Cumulative distribution functions are denoted by uppercase letter $\cdf$: $\cdf(x) := \int_{-\inf}^x f(u) \dd u$;
probability measures for subsets $A$ of a sample space $\Omega$ are denoted by uppercase $\p$, i.e.\ $\p(A) = \int_A f(u) \dd u$.
