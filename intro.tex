...
special sets: real numbers $\reals$, positive real numbers $\reals_{>0}$

Prior/posterior probability distributions(on parameters) are usually denoted by lowercase letter $p$,
while sample models (distributions on observable quantities) are denoted by lowercase letter $f$:
$f(x)$, $p(\theta)$
Expectation and variance of a random quantity $X$: $\E[X]$, $\V(X)$.
Usually, quantities identifying the distribution
with respect to which expectation and variance are calculated
are added in the argument, seperated by a vertical line: $f(x\mid\theta)$, $\E[\theta\mid\nz, \yz]$.
...
Like in frequentist Neyman-Pearson testing, these are often denoted by $H_0$ and $H_1$,
...
The problem (?) with point-null hypotheses (Robert), but model selection.
...
\end{align*}
for all appropriately sized matrices $\mbf{A}$ and column vectors $\vec{a}$.
