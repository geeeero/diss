...
where we will give an overview on the contents of this thesis,
...
There, we will describe the basics of statistical inference using parametric models,
...
serves to show the potential of generalised Bayesian inference using sets of conjugate priors.
...
giving a superstructure for some notable imprecise probability models
which have been central to the development and application of imprecise probability methods
...
and a number of models that can be subsumed under this framework are discussed with respect to
...
and compares these to models of the framework from Section~\ref{sec:generalmodel}.
The remainder of Chapter~\ref{cha:imprecisebayes-conjugate}
reproduces two works that suggest novel models
that provide a sophisticated handling of prior-data conflict
(Sections~\ref{sec:jstp} and \ref{sec:isipta11}),
and gives a short overview on a software implementation (Section~\ref{sec:luck}).
...
The Appendix (Chapter~\ref{cha:appendix}) provides some supplemental material. 
Section~\ref{sec:festschrift} studies \pdc\ sensitivity in Bayesian linear regression,
while Section~\ref{sec:boatshape} describes
an informal rationale and some first technical results
...
and illustrates its application to the Normal-Normal and Dirichlet-Multinomial models. % in more detail. 
Section~\ref{sec:luck} gives a short overview on a software implementation of \nymodel s,
the add-on package \texttt{luck} for the statistical programming environment \textbf{R} \parencite{2013:r}.
Finally,
...
Section~\ref{sec:diri-multi} is instead based on \textcite{Walter2012b}.
%
...
except Section~\ref{sec:imprecisebayes}, which is based on \textcite[\S\S 4.2, 4.4]{itip-statinf}.
...
%
Section~\ref{sec:motivation} was newly devised for this thesis,
under use of \textcite[\S 2.3]{itip-statinf}
for the last paragraph in Section~\ref{sec:motivation:bayesian-foundational},
and Sections~\ref{sec:motivation:near-ignorance} and \ref{sec:motivation:pdc}.
%
Section~\ref{sec:ip-alternatives} uses parts of \textcite[\S\S 2.2, 2.4]{itip-statinf},
...
using some minor parts of \textcite[\S\S 4.2, 4.4]{itip-statinf}.
...
and was newly written for this thesis.
%Author and maintainer of this add-on package \texttt{luck} \parencite{luck-package} is Gero Walter,
...
Here, it was also possible to add a few explanatory paragraphs
...
e.g., $\cdf(x) := \int_{-\infty}^x f(u) \dd u$, or $\cdf(\theta) := \int_{-\infty}^\theta p(\psi) \dd \psi$;
...
are added in the argument, separated by a vertical line, as in $f(x\mid\theta)$, or $\E[\theta\mid\nz, \yz]$.
...
\begin{align*}
l(\hat{\theta}, \theta) &= \begin{cases} 2  q    (\hat{\theta}-\theta) & \text{if } \hat{\theta}-\theta \ge 0 \\
                                         2 (q-1) (\hat{\theta}-\theta) & \text{if } \hat{\theta}-\theta < 0 \end{cases}\,,
\end{align*}
...
which had prompted the reactors to shutdown automatically, and in doing so,
