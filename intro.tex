...
random variables by uppercase letters ($X$, $\vec{X}$);
...
the real numbers by $\reals$, positive real numbers by $\reals_{>0}$,
and the space of $q$-dimensional tuples of real numbers by $\reals^q$.

In the Bayesian setting, prior and posterior probability distributions (on parameters) are usually denoted by lowercase letter $p$,
while sample models (distributions on observable quantities) are denoted by lowercase letter $f$:
$f(x)$, $p(\theta)$.

Cumulative distribution functions are denoted by uppercase letter $\cdf$: $\cdf(x) := \int_{-\infty}^x f(u) \dd u$;
probability measures, e.g.\ for subsets $A$ of a sample space $\Omega$, are denoted by uppercase $\p$,
i.e.\ $\p(A) = \sum_{\omega \in A} f(\omega)$ if $\Omega$ is countable,
and $\p(A) = \int_A f(\omega) \dd \omega$ otherwise.
