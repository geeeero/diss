...
With the basics of Bayesian inference based on parametric distributions
covered in Section~\ref{sec:bayes-inference},
the motivating example in Section~\ref{sec:commoncause},
...
presenting the general framework for generalised Bayesian inference using sets of priors.
...
are the central topics guiding our assessments of the specific imprecise probability models covered
...
and the range of models that can be subsumed under this framework are discussed with respect to
...
and compares these to the model framework from Section~\ref{sec:generalmodel}.
The remainder of Chapter~\ref{cha:imprecisebayes-conjugate}
reproduces two works that suggested important models %***geared at***
...
\end{itemize}
not main (only Sec.~\ref{sec:diri-multi}): \textcite{Walter2012b}

sources, which papers where***
...
Sec.~\ref{sec:commoncause} is based on %parts of
...
Sec.~\ref{sec:motivation} was devised for this thesis,
...
Section~\ref{sec:alternatives} was devised for this thesis, using some very minor parts of \textcite[\S 4.2, \S 4.4]{itip-statinf}.***
...
Author and maintainer of this add-on package ***citation*** is Gero Walter,
