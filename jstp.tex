...
%\includegraphics*[height=\textwidth,angle=-90,bb = 170 90 415 770]{../../../R/Workspace/lokal/jstp-paper_nv_nfest_02-071220.ps}%
%}
%Illustration of an update step in Example~1a in case of prior-data conflict
...
%Consider an observation again of size $10$ but with \gwc{$\tilde{\tau}(x) = 8$},
...
imprecise prior,  ${\cal M}_{|\x}$ is the corresponding posterior. It is obtained %here
...
$\ttau(\x) \notin \YZ$.
%%GW081202%% When no prior-data conflict occurs, only $\ol{n}\uz$ is used for updating ${\cal Y}\uz$.
...
$\ttau(\x) > \yzu$, where $\nzu$ is still used to
calculate $\ynl$, but $\nzl$ to calculate $\ynu$.}
%****hier noch mehr Text sonst fliegt alles auseinander***
