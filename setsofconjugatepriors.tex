...
This section attempts to give a systematic overview on imprecise probability models
...
As a direct extension of the models discussed in Section~\ref{sec:generalmodel},
it may be stressed that their central properties rely on the specific form of the update step \eqref{eq:canonicalupdate} only.
...
This is elaborated in Section~\ref{sec:jstp} \parencite{Walter2009a} by the definition of so-called LUCK-models,
which were already used in \textcite{Walter2007a} to generalise Bayesian linear regression.
...
***par from \textcite[\S 4.4]{itip-statinf}:

In Section~\ref{sec:generalmodel},
we have discussed a specific class of models based on sets of conjugate priors,
and illustrated in an exemplary way the main ideas and potential of generalized Bayesian inference.
Other work on exponential family sampling models includes \textcite{1993:coolen}, ***discuss in detail???***
who uses a different type of imprecise conjugate prior densities,
...
Most models in generalized Bayesian inference rely on a precise sampling model,
