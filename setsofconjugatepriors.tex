...
and models discussed in the literature that can be subsumed under this framework are described. % named/given/enumerated.
%
...
f_m(\x) &= \int_\Theta f(\x\mid\vartheta) p_m(\vartheta\mid \nz_1,\yz_1, \nz_2,\yz_2, \alpha) d\vartheta \\
        &=    \alpha  \int_\Theta f(\x\mid\vartheta) p(\vartheta\mid \nz_1,\yz_1) d\vartheta 
         + (1-\alpha) \int_\Theta f(\x\mid\vartheta) p(\vartheta\mid \nz_2,\yz_2) d\vartheta \\
        &=    \alpha  \int_\Theta f_1(\x) p(\vartheta\mid \nn_1,\yn_1) d\vartheta 
         + (1-\alpha) \int_\Theta f_2(\x) p(\vartheta\mid \nn_2,\yn_2) d\vartheta \\
        &= \alpha f_1(\x) + (1-\alpha) f_2(\x) \,,
\end{align*}
the posterior coresponding to $p_m(\vartheta\mid \nz_1,\yz_1, \nz_2,\yz_2, \alpha)$,
derived via Bayes' Rule, % \eqref{eq:bayesrule},
...
p_m(\vartheta\mid \nz_1,\yz_1, \nz_2,\yz_2, \alpha, \x) } \hspace*{8ex} & \nonumber\\
 &= \frac{f(\x\mid\vartheta)}{f_m(\x)} \Big(
     \alpha\,  p(\vartheta\mid\nz_1,\yz_1) +
  (1-\alpha)\, p(\vartheta\mid\nz_2,\yz_2) \Big) \nonumber\\
 &=  \alpha\,  \frac{f(\x\mid\vartheta)}{f_m(\x)} p(\vartheta\mid\nz_1,\yz_1) +
  (1-\alpha)\, \frac{f(\x\mid\vartheta)}{f_m(\x)} p(\vartheta\mid\nz_2,\yz_2) \nonumber\\
 &=  \alpha\,  \frac{f_1(\x)}{f_m(\x)} p(\vartheta\mid\nn_1,\yn_1) +
  (1-\alpha)\, \frac{f_w(\x)}{f_m(\x)} p(\vartheta\mid\nn_2,\yn_2) \nonumber\\
 &=: p_m(\vartheta\mid \nn_1,\yn_1, \nn_2,\yn_2, \alpha^*)\,,
\label{sec:posteriormixture}
\end{align}
where
\begin{align*}
\alpha^* &= \alpha\, \frac{f_1(\x)}{f_m(\x)}
          = \frac{\alpha\, f_1(\x)}{\alpha\, f_1(\x) + (1-\alpha)\, f_2(\x)}\,. 
\end{align*}
The posterior of the $\alpha$-mixture distribution,
based on parametric priors with parameters $(\nz_1,\yz_1)$ and $(\nz_2,\yz_2)$,
is thus the $\alpha^*$-mixture of two parametric posteriors,
...
where $\alpha$ varies in $[0, 1]$.
As for any $\alpha \in [0,1]$, the corresponding $\alpha^*$ is again in $[0,1]$,
and it holds that $\{\alpha^* \mid \alpha \in [0,1]\} = [0,1]$,
...
as the convex hull of the parametric posteriors %updated parametric distributions.
with $(\nn,\yn) \in \PN$.

When the credal set $\MZ$ is taken to contain all finite mixtures of parametric priors,
%convex combinations (i.e., finite mixtures) of parametric priors,
...
even if $\MN$ is taken to contain convex combinations.} %of the parametric distributions.}
...
%Graph taken from \textcite[p.~267]{2009:walteraugustin}.}
%\textbf{comment by Frank: depict also prior/posterior parameter sets?}}
%\end{figure}
