...
\frame{\frametitle{Model Discussion}
\begin{itemize}%[<+->]
...
\item<4-> Set shape is crucial modeling choice:\\ %A range of $\nzg$ values is needed for \pdc\ reaction
...
% \begin{itemize}
 ...
 implementation: \textbf{R} package \texttt{luck}
% \end{itemize}
\item<7-> Other set shapes are possible, but may be more difficult to handle %elicit
%\item<8-> Prior information may be such that range of $\yzr$ changes with $\nzg$ (or vice versa)
\end{itemize}
%\alert{generalised iLUCK-models}
...
\frame{\frametitle{Parameter Set Shape for Strong Prior-Data Agreement}
...
\item<2-> Sets of conjugate priors maintain advantages \& mitigate issues %find a sweet spot in between
 ...
 (Walter \& Augustin 2009: \emph{generalised iLUCK-models}, \texttt{luck})
