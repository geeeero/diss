...
\frame{\frametitle{Prior-Data Conflict}
...
\hfill $=$ defining a conglomerate of probability statements (on $\vartheta$).\\[1.5ex]

Bayesian theory lacks the ability to specifiy the degree of uncertainty in these probability statements.\\[1.5ex]
...
\pause
\hspace*{6ex}\fbox{\parbox[t]{0.9\textwidth}{add \alert{imprecision} as new model dimension:\\
\alert{sets of priors} model uncertainty about probability statements}}
...
\begin{itemize}%[<+->]
\item<1-> Need a range of $\nzg$ values for \pdc\ reaction
\item<2-> Other set shapes are possible, but may be more difficult to elicit
\item<3-> Prior information may be such that range of $\yzr$ changes with $\nzg$ (or vice versa)
\item<4-> \emph{Near-ignorance priors}: $\PZc$ such that prior inferences are nearly \emph{vacuous},
...
%\vspace*{-7ex}%
\begin{center}
\includegraphics[trim = 15mm 45mm 25mm 60mm, clip, width=\textwidth]{../R/boatshape-posterior-normal} %[scale=0.63]
\end{center}}
...
\begin{itemize}
\item fukushima picture
\item common-cause failure box
\item alpha factor model uses Dirichlet-Multinomial model
\item informative priors (elicited from experts), as data are luckily sparse (zero counts for multi-component failures)
\item non-informative priors are unsatisfactory
(posterior very sensitive to chice of prior, which non-informative prior should we adopt?)
\item Troffaes, Walter \& Kelly (2013): adapt Imprecise Dirichlet model (IDM, Walley 1996)
\item \pdc
...
\item<2-> Sets of conjugate priors maintain advantages \& mitigate issues %find a sweet spot in between
 ...
 \item Shape for less imprecision in case of strong prior-data agreement is in development
...
\nocite{2006:evans,1996:walley::idm,1991:walley,2005:quaeghebeurcooman-short,Walter2009a-short,luck-package,Troffaes2013a}
%1994:hoyland,1965:good,2011:kelly:atwood,1996:atwood,}
