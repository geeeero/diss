...
\uncover<1->{%
\begin{block}{Bayesian inference procedure}
\centerline{prior + likelihood = posterior}
using Bayes' Rule\\
All inferences are based on the posterior\\
(e.g., point estimate = posterior expectation, \ldots)
\end{block}}
...
\item<7-> \alert{Near-ignorance priors}: $\PZc$ such that prior inferences are nearly \emph{vacuous},
