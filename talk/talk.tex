...
\frame{\frametitle{Parameter Set Shapes ***pdfplots normal?}
...
\uncover<2>{\node {\includegraphics[scale=0.75]{../R/shape1.pdf}};
            \draw[-stealth,very thick] (-1,-0.4) -- (2.3,1.1) node [above,midway,sloped] {9 out of 10};}
\uncover<3>{\node {\includegraphics[scale=0.75]{../R/shape2.pdf}};
            \draw[-stealth,very thick] ( 0,-0.5) -- (2.3,1.1) node [above,midway,sloped] {9 out of 10};}
...
\begin{itemize}
\item fixed $\nzg$ very easy model, but no additional imprecision in case of \pdc
\item $[\nzlg, \nzug]$: flexible weighing of prior and data 
\item need a range of $\nzg$ values for \pdc\ reaction
\item other set shapes possible, but more difficult to elicit
\item near-ignorance stuff?
\end{itemize}
}

\frame{\frametitle{Prior-Data Conflict and Parameter Set Shapes}
\begin{itemize}
\item Ausblick: boatshape (extra slides?)***
\end{itemize}
}
