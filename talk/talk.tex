...
\frame{\frametitle{Generalised Bayesian Inference with Sets of Priors}
...
of inference procedure (as in standard Bayesian inference)
\end{block}
...
where $\PNc = \big\{(\nng,\bynr) \colon (\nzg,\byzr) \in \PZc \big\}$.\\
%Set of posteriors = 
Finite convex mixtures of canonical conjugate posteriors\\ defined by set of updated parameters $\PNc$
\end{block}
...
Parameter set $\PZc$ defines set of priors

Bounds for quantities being linear in $p(\psib\mid\nng,\bynr)$\\
(expectations $\E[g(\psib)\mid\nng,\bynr]$, \ldots)
are attained at canonical conjugate posteriors and thus \alert{straighforward to calculate}
...
\begin{itemize}
\item Parameter set $\PZc$ defines set of priors\
\item Shape of $\PZc$ influences model behaviour
\end{itemize}
