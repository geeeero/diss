...
IDM (Walley 1996), Quaghebeur \& de Cooman (2005)
 \begin{itemize}
 \item simpe model description, posterior parameter set has same form $\PNc = \nng \times [\ynlr, \ynur]$
 \item optimise over $[\ynlr, \ynur]$ only %(often, posteriors are stoch.\ ordered along $\ynr$)
 \item no \pdc\ reaction: same imprecision as without conflict (just like precise prior)
 \end{itemize}
\item $\PZc = [\nzlg,\nzug] \times [\yzlr, \yzur]$:\ %\thenthen\
Walley (1991, \S 5.4.3), Walter \& Augustin (2009) \emph{LUCK-models}
 \begin{itemize}
 ...
 \item posterior parameter sets $\PNc$ have non-trivial forms (banana / spotlight)
 \item however, closed form for $\min / \max_{\PNc} \ynr$
...
but posterior inferences are informative
 \begin{itemize}
 \item IDM (Walley 1996): range of $\yzjr{j} = (0,1)\ \forall j$
 \item Benavoli \& Zaffalon (2012): range of $\yzr = (-\infty,+\infty)$ while $\nzug$ decreasing with $\yzr$ %$\nzg$ bounded above
 to avoid $\nzg |\yzr| = \infty$ \thenthen\ vacuous posterior inferences
 \end{itemize}
\end{itemize}
...
Work in progress: parameter set shape enabling
\begin{itemize}
\item additional imprecision in case of \pdc\ (as before)
\item less imprecision for \emph{strong prior-data agreement}
\end{itemize}
using other parametrisation of priors suggested by Mik Bickis (in preparation)
}
...
\begin{itemize}
\item Conjugate priors are convenient but have some pitfalls
 \begin{itemize}
 ...
 \end{itemize}
\item Sets of conjugate priors use advantages \& avoid pitfalls %find a sweet spot in between
 \begin{itemize}
