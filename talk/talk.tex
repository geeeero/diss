...
\begin{itemize}
\item Parameter set $\PZc$ defines set of priors $\MZ$
\item Parameter set $\PNc$ defines set of posteriors $\MN$
\item $\PZc \to \PNc$ is easy ($\nng = \nzg + n$, $\ynr = \dots$)
\item Quantities linear in $p(\psib\mid\nng,\bynr)$
(e.g., $\E[g(\psib)\mid\nng,\bynr]$):\\
\then\ bounds attained at ``pure'' posteriors $p(\psib\mid\nng,\bynr)$\\
\then\ \alert{straighforward to calculate:} optimise over $(\nng,\bynr)$ only
\item Often, optimising over $(\nng,\bynr)$ is also easy:\\
posterior guess for $\frac{\tau(\x)}{n}$ (think: $\bar{\x}$) = $\ynr$\\
\then\ \alert{analytic calculations}
\end{itemize}
